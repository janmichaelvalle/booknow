...
export const InteractiveStepperItem = React.forwardRef<HTMLDivElement, IStepperItemProps>(
    ({ children, completed = false, disabled = false, rejected = false, className = '', ...props }, ref) => {
        ...
        const contextValue = useMemo(
            ...
            [stepIndex, state, disabled, rejected],
        );
...
export const InteractiveStepperIndicator = React.forwardRef<HTMLDivElement, IStepperIndicatorProps>(
    ({ className = '', ...props }, ref) => {
        ...
        const { state, stepIndex, rejected } = stepperItemContext
            ;

        const classes = {
            completed:
                'bg-primary text-primary-foreground shadow h-9 w-9 p-1 flex flex-col items-center text-center gap-1 z-10 rounded-full shrink-0',
            active:
                'bg-primary text-primary-foreground shadow h-9 w-9 p-1 flex flex-col items-center text-center gap-1 z-10 rounded-full shrink-0 ring-2 ring-ring ring-offset-2 ring-offset-background',
            inactive:
                'border border-input bg-background shadow-sm h-9 w-9 p-1 flex flex-col items-center text-center gap-1 z-10 rounded-full shrink-0',
        }
        ...
        return (
            <div ref={ref} className={builtIndicatorClasses} {...props}>
                {rejected ? 'X' : state === 'completed' ? <Check className="h-4 w-4" /> : stepIndex}
            </div>
...
export const InteractiveStepperTrigger = React.forwardRef<HTMLDivElement, IStepperTriggerProps>(
    ({ children, className = '', ...props }, ref) => {
        ...
        return (
            <div ref={ref} className={triggerClasses} {...props}>
                {children}
            </div>
        )
    },
